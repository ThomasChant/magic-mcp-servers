...
if (!hasDeepSeek && !hasGemini) {
  console.error('❌ 缺少AI API配置: 至少需要配置DEEPSEEK_API_KEY或GEMINI_API_KEY之一');
 ...
 * Gemini API调用工具
 */
class GeminiAPI extends AIAPIClient {
  protected modelType = AIModel.GEMINI;

  async callAPI(prompt: string, retries: number = 3, timeoutMs: number = 30000): Promise<string> {
    apiUsageStats.gemini.attempts++;
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        Logger.info(`Gemini API调用尝试 ${attempt}/${retries}`);

        // 创建超时控制器
        const controller = new AbortController();
        const timeoutId = setTimeout(() => {
          controller.abort();
          Logger.warning(`Gemini API调用超时 (${timeoutMs}ms), 尝试 ${attempt}/${retries}`);
        }, timeoutMs);
        ...
        const response = await fetch(`${GEMINI_API_URL}?key=${GEMINI_API_KEY}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `你是一个专业的README文档分析师，擅长从技术文档中提取结构化信息。请严格按照要求的JSON格式返回结果。\n\n${prompt}`
              }]
            }],
            generationConfig: {
              temperature: 0.1,
        ...
        if (!data.candidates || data.candidates.length === 0) {
          throw new Error('Gemini API返回数据格式错误: 缺少candidates');
        }

        const result = data.candidates[0].content.parts[0].text;
        Logger.success(`Gemini API调用成功 (尝试 ${attempt}/${retries})`, {
          responseLength: result.length,
          responsePreview: result.substring(0, 200) + '...'
        });

        apiUsageStats.gemini.successes++;
        return result;

      } catch (error) {
        // 特殊处理超时错误
        if (error instanceof Error && error.name === 'AbortError') {
          Logger.error(`Gemini API调用超时 (尝试 ${attempt}/${retries})`, `超时时间: ${timeoutMs}ms`);
        } else {
          Logger.error(`Gemini API调用失败 (尝试 ${attempt}/${retries})`, error);
        }
        ...
        if (attempt < retries) {
          ...
          apiUsageStats.gemini.failures++;
          if (error instanceof Error && error.name === 'AbortError') {
            throw new Error(`Gemini API调用超时，已重试 ${retries} 次，每次超时 ${timeoutMs}ms`);
          } else {
            throw new Error(`Gemini API调用失败，已重试 ${retries} 次: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      }
    }
...
function selectRandomAPI(): AIAPIClient {
  ...
  if (hasGemini) {
    availableAPIs.push(new GeminiAPI());
  }
...
async function saveToDatabase(
  ...
  Logger.info(`保存分析结果到数据库: ${projectName}`, {
    ...
    apiReference: {
      ...
      usageExamplesCount: apiReference.usage_examples.length
    }
  });
  ...
  try {
    const { error } = await supabase
      .from('server_readmes')
      .update({
        extracted_installation: installation,
        extracted_api_reference: apiReference,
...
async function processReadmeWithAI(readme: ReadmeRecord, currentIndex: number, totalCount: number): Promise<void> {
  ...
  try {
    await withTimeout(async () => {
      ...
      Logger.info(`步骤2: 启动AI并行分析 - ${readme.project_name}`);
      const analysisStartTime = Date.now();
      
      const [extractedInstallation, extractedApiReference] = await Promise.all([
        analyzeInstallationWithAI(readme.raw_content, readme.project_name),
        analyzeAPIReferenceWithAI(readme.raw_content, readme.project_name)
      ]);
      ...
      Logger.success(`步骤2完成: AI分析完成 - ${readme.project_name}`, {
        ...
        installationMethods: extractedInstallation.methods.length,
        clientConfigs: extractedInstallation.client_configs.length,
        apiTools: extractedApiReference.tools.length
      });
      ...
      await saveToDatabase(readme.id, readme.project_name, extractedInstallation, extractedApiReference);
...
async function main(): Promise<void> {
  ...
  Logger.info('环境配置检查', {
    ...
    geminiApiUrl: hasGemini ? GEMINI_API_URL : 'N/A'
  });
