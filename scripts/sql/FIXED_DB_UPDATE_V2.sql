-- FIXED V2: Update servers_with_details view to include voting data
-- This fixes the "column reference github_url is ambiguous" error
-- Updated to avoid column ambiguity by explicitly selecting columns
...
SELECT 
    -- Explicitly select all columns from mcp_servers to avoid ambiguity
    s.id,
    s.name,
    s.owner,
    s.slug,
    ...
    s.github_url,  -- Explicitly use github_url from mcp_servers table
    s.demo_url,
    s.docs_url,
    s.repository_owner,
    s.repository_name,
...
GROUP BY s.id, s.name, s.owner, s.slug, s.description_zh_cn, s.description_en, s.description_zh_tw, 
         s.description_fr, s.description_ja, s.description_ko, s.description_ru, s.full_description, 
         s.icon, s.category_id, s.subcategory_id, s.featured, s.verified, s.featured_icon, 
         s.featured_rating, s.featured_badge, s.featured_badge_color, s.github_url, s.demo_url, 
         s.docs_url, s.repository_owner, s.repository_name, s.stars, s.forks, s.watchers, 
         s.open_issues, s.last_updated, s.repo_created_at, s.quality_score, s.quality_documentation, 
         s.quality_maintenance, s.quality_community, s.quality_performance, s.complexity, s.maturity, 
         s.downloads, s.dependents, s.weekly_downloads, s.platforms, s.node_version, s.python_version, 
...
COMMENT ON VIEW servers_with_details IS 'Complete server view with voting data, tags, and category information - Fixed column ambiguity';
