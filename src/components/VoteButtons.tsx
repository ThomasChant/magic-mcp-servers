...
/**
 * VoteButtonsBatch - Uses only batch providers, no individual queries
 * Optimized for pages with multiple servers (like servers listing)
 */
const VoteButtonsBatch: React.FC<VoteButtonsProps> = ({
    serverId,
    size = 'md',
    className = '',
    showScore = true
...
/**
 * VoteButtonsIndividual - Uses only individual queries, no batch providers
 * Optimized for single server pages or when batch providers aren't available
 */
const VoteButtonsIndividual: React.FC<VoteButtonsProps> = ({
    serverId,
    size = 'md',
    className = '',
    showScore = true
...
export const DetailedVoteButtons: React.FC<VoteButtonsProps & { 
    ...
    // Use the same smart detection as VoteButtons
    const { score: batchScore, hasBatchProvider: hasScoreBatch } = useBatchScore(props.serverId);
    const { hasBatchProvider: hasVoteBatch } = useBatchUserVote(props.serverId);
    
    // Get server score using the same logic as VoteButtons
    const { data: individualScore } = useServerScore(props.serverId, !(hasScoreBatch && hasVoteBatch));
    const serverScore = (hasScoreBatch && hasVoteBatch) ? batchScore : individualScore;
...
/**
 * VoteButtons - Smart wrapper component that detects batch providers and chooses optimal implementation
 * Uses VoteButtonsBatch when batch providers are available, VoteButtonsIndividual otherwise
 */
const VoteButtons: React.FC<VoteButtonsProps> = (props) => {
    // Check if batch providers are available
    const { hasBatchProvider: hasScoreBatch } = useBatchScore(props.serverId);
    const { hasBatchProvider: hasVoteBatch } = useBatchUserVote(props.serverId);
    
    // Use batch component when both batch providers are available
    if (hasScoreBatch && hasVoteBatch) {
        return <VoteButtonsBatch {...props} />;
    }
    
    // Fall back to individual component
    return <VoteButtonsIndividual {...props} />;
};
